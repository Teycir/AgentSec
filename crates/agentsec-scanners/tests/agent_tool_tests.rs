...
fn policy_detector_flags_forbidden_tool_with_no_explicit_assertion() {
    // The suite author only wrote an assertion about "search_docs" being
    // called; the agent additionally (and silently, from the suite's
    // perspective) called a forbidden tool. The policy-driven built-in
    // detector should catch what the assertion didn't.
    let test = base_test(
        "t1",
...
fn require_human_approval_tools_are_not_auto_flagged() {
    // send_email is in require_human_approval, not forbidden_tools - the
    // policy-driven detector must not treat it as an automatic violation,
    // since this scanner has no way to know whether approval happened.
